...
class SelectTableTablePrompt extends Base {
  /**
  ...
  render(error) {
    ...
    const table = new Table({
      ...
      head: [
        chalk.reset.dim(``),
          ...
          this.columns.pluck("name").map((name) => chalk.reset.bold(chalk.yellow(name)))
      ),
      style: {
        'padding-left': 1,
        'padding-right': 1,
    ...
    this.rows.forEach((row, rowIndex) => {
      ...
      table.push({
        ['']: Object.values(row).map((value) => value === undefined ? "" : chalkModifier(value)),
      });
    });
...
module.exports = SelectTableTablePrompt;
